...
class sampler():
    def __init__(self, use_rippe, S_o_A_frags, collector_id_repeats, frag_dispatcher,
                 ...
                 norm_vect_accu,
                 sub_candidates_dup, sub_candidates_output_data,
                 S_o_A_sub_frags, sub_collector_id_repeats, sub_frag_dispatcher,
                 sparse_matrix,
                 mean_value_trans,
                 n_iterations, is_simu,
        ...
        self.param_simu_rippe = np.dtype([('kuhn', np.float32), ('lm', np.float32), ('c1', np.float32),
                                          ('slope', np.float32),
                                          ('d', np.float32), ('d_max', np.float32), ('fact', np.float32),
                                          ('v_inter', np.float32)], align=True)

        self.param_simu_exp = np.dtype([('d0', np.float32),('d_max', np.float32),
                                        ('alpha_0', np.float32), ('alpha_1', np.float32), ('fact', np.float32),
                                        ('v_inter', np.float32)], align=True)
        if self.use_rippe:
        ...
        seed = 1
        self.rng_states = cuda.mem_alloc(self.n_generators*characterize.sizeof('curandStateXORWOW',
    ...
    def setup_all_gpu_struct(self,):
        ...
        for k in xrange(0, self.n_tmp_struct):
        ...
        self.gpu_info_blocks = ga.to_gpu(np.zeros((int(self.n_non_zero / self.size_block_4_sub + 1),), dtype=self.int3))
        ...
        self.gpu_old_2_new_id_c = ga.zeros((self.n_new_sub_frags + self.n_new_sub_frags / 10,), dtype=np.int32) # security length

        if self.active_insert_blocks:
            # self.gpu_list_bounds = ga.to_gpu(ary=np.array(range(1, self.n_insert_blocks + 1), dtype=np.int32))
            list_size = np.array([1, 3, 5, 10, 20, 50, 200, 200], dtype=np.int32)
            self.max_bounds_insert = list_size[:self.n_insert_blocks].max() * np.int32(np.round(self.S_o_A_frags['sub_len'].mean())+1) # approximation of sub size to extract in full matrix
    ...
    def setup_thrust_modules(self):
        ...
        nvcc_function_sort_by_keys_zip = cgen.FunctionBody(
            cgen.FunctionDeclaration(cgen.Value('void', 'my_sort_zip'),
            ...
            cgen.Block([cgen.Statement('thrust::device_ptr<int> thrust_ptr((int*)input_ptr_keys)'),
                        ...
                        cgen.Statement('thrust::tuple< thrust::device_ptr<int>, thrust::device_ptr<int> > keytup_begin = thrust::make_tuple(thrust_ptr_valsa, thrust_ptr_valsb)'),
                        cgen.Statement('thrust::zip_iterator<thrust::tuple<thrust::device_ptr<int>, thrust::device_ptr<int> > > first = thrust::make_zip_iterator(keytup_begin)'),
        ...
        nvcc_function_sort_by_keys_zip_cmplex = cgen.FunctionBody(
            cgen.FunctionDeclaration(cgen.Value('void', 'my_sort_zip_cmplex'),
                                [cgen.Value('CUdeviceptr', 'input_ptr_keys_a'),
                                 cgen.Value('CUdeviceptr', 'input_ptr_keys_b'),
                                 cgen.Value('int', 'length'),
                                 cgen.Value('CUdeviceptr', 'input_ptr_vals')]),
            cgen.Block([cgen.Statement('thrust::device_ptr<int> thrust_ptr_v((int*)input_ptr_vals)'),
                        ...
                        cgen.Statement('thrust::tuple< thrust::device_ptr<int>, thrust::device_ptr<int> > keytup_begin = thrust::make_tuple(thrust_ptr_keysa, thrust_ptr_keysb)'),
                        cgen.Statement('thrust::zip_iterator<thrust::tuple<thrust::device_ptr<int>, thrust::device_ptr<int> > > first = thrust::make_zip_iterator(keytup_begin)'),
        ...
        nvcc_function_sort_by_keys_simple = cgen.FunctionBody(
            cgen.FunctionDeclaration(cgen.Value('void', 'my_sort_simple'),
                                [cgen.Value('CUdeviceptr', 'input_ptr_keys'),
                                 cgen.Value('int', 'length'),
                                 cgen.Value('CUdeviceptr', 'input_ptr_vals')]),
            cgen.Block([cgen.Statement('thrust::device_ptr<int> thrust_ptr((int*)input_ptr_keys)'),
                        cgen.Statement('thrust::device_ptr<int> thrust_ptr_v((int*)input_ptr_vals)'),
                        cgen.Statement('thrust::stable_sort_by_key(thrust_ptr, thrust_ptr+length, thrust_ptr_v, thrust::greater<int>())')]))
        #Add declaration to nvcc_mod
        #Adds declaration to host_mod as well
        ...
        nvcc_function_prefix_sum = cgen.FunctionBody(
            cgen.FunctionDeclaration(cgen.Value('void', 'my_prefix_sum'),
                                [cgen.Value('CUdeviceptr', 'input_ptr_vals'),
                                 cgen.Value('int', 'length'),]),
            cgen.Block([cgen.Statement('thrust::device_ptr<int> thrust_ptr_v((int*)input_ptr_vals)'),
        ...
        host_statements_sort_zip = [
            ...
            #Get data pointer
            'CUdeviceptr ptr_keys = extract<CUdeviceptr>(gpu_array_keys.attr("gpudata"))',
            'CUdeviceptr ptr_valsa = extract<CUdeviceptr>(gpu_array_valsa.attr("gpudata"))',
            'CUdeviceptr ptr_valsb = extract<CUdeviceptr>(gpu_array_valsb.attr("gpudata"))',
        ...
        host_mod.add_function(
            cgen.FunctionBody(
                cgen.FunctionDeclaration(cgen.Value('object', 'sort_by_keys_zip'),
        ...
        host_statements_sort_zip_cmplex = [
            ...
            # 'int length = extract<int>(shape[0])',
            #Get data pointer
            'CUdeviceptr ptr_keys_a = extract<CUdeviceptr>(gpu_array_keys_a.attr("gpudata"))',
            'CUdeviceptr ptr_keys_b = extract<CUdeviceptr>(gpu_array_keys_b.attr("gpudata"))',
            'CUdeviceptr ptr_vals = extract<CUdeviceptr>(gpu_array_vals.attr("gpudata"))',
            #Call Thrust routine, compiled into the CudaModule
        ...
        host_mod.add_function(
            cgen.FunctionBody(
                cgen.FunctionDeclaration(cgen.Value('object', 'sort_by_keys_zip_cmplex'),
        ...
        host_mod.add_function(
            cgen.FunctionBody(
                cgen.FunctionDeclaration(cgen.Value('object', 'sort_by_keys_simple'),
                                    [cgen.Value('object', 'gpu_array_keys'),
                                     cgen.Value('int', 'n_vals'),
                                     cgen.Value('object', 'gpu_array_vals'),]),
        ...
        host_mod.add_function(
            cgen.FunctionBody(
                cgen.FunctionDeclaration(cgen.Value('object', 'prefix_sum'),
                                    [cgen.Value('object', 'gpu_array_vals'),
                                     cgen.Value('int', 'n_vals'),]),
        ...
        gcc_toolchain.cflags.append("-D_FORCE_INLINES")
        nvcc_toolchain = codepy.toolchain.guess_nvcc_toolchain()
        nvcc_toolchain.cflags.append("-D_FORCE_INLINES")
    ...
    def create_gpu_struct(self, data):
        if data == None:
            gpu_vect = GPUStruct([(np.int32, '*pos', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  (np.int32, '*sub_pos', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  ...
                                  (np.int32, '*sub_len', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  ...
                                  (np.int32, '*sub_l_cont', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  (np.int32, '*l_cont_bp', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  (np.int32, '*ori', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  (np.int32, '*rep', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  (np.int32, '*activ', np.zeros((self.n_new_frags,), dtype=np.int32)),
                                  (np.int32, '*id_d', np.zeros((self.n_new_frags,), dtype=np.int32))])

        else:
            gpu_vect = GPUStruct([(np.int32, '*pos', np.copy(data['pos'])),
    ...
    def sparse_data_2_gpu(self,):
        ...
        # self.gpu_sp_no_rep_indptr = ga.to_gpu(ary=self.mat_without_repeats.indptr)
        ...
        total_mem_sparse = self.sparse_matrix.data.nbytes + self.sparse_matrix.indptr.nbytes + \
                           self.sparse_matrix_coo.row.nbytes + self.sparse_matrix_coo.col.nbytes

        # self.gpu_sub_sp_data = ga.to_gpu(ary=self.sub_sampled_sparse_matrix.data)
        # self.gpu_sub_sp_indptr = ga.to_gpu(ary=self.sub_sampled_sparse_matrix.indptr)
        # self.gpu_sub_sp_indices = ga.to_gpu(ary=self.sub_sampled_sparse_matrix.indices)
        # self.gpu_sub_sp_n_indices = np.int32(self.sub_sampled_sparse_matrix.indices.shape[0])

        # total_mem_sparse += self.sub_sampled_sparse_matrix.data.nbytes + self.sub_sampled_sparse_matrix.indptr.nbytes + \
        #                     self.sub_sampled_sparse_matrix.indices.nbytes
        print "total mem used by sparse data = ", np.float32(total_mem_sparse) / 10**6.
    ...
    def approx_single_likelihood_on_zeros(self,):
        ...
        t0 = time.time()
        start.record()
        self.kern_eval_likelihood_zeros(self.gpu_vect_id_c,
                                        self.gpu_vect_s_tot,
                                        self.gpu_vect_pos,
                                        self.gpu_vect_len,
                                        self.gpu_param_simu,
        ...
        # print "CUDA clock execution timing (compute likelihood on zeros): ", secs
        self.val_on_zero_intra = self.gpu_likelihood_on_zeros.get()[0] * self.log_e
        self.n_vals_intra = self.gpu_n_vals_intra.get()[0]
        t1 = time.time()
        self.val_on_zero_inter = self.log_e * (self.n_pixl_sub_mat - self.n_vals_intra) *\
                                         -1.0 * self.param_simu['v_inter'][0]
    ...
    def approx_single_likelihood_on_zeros_nuisance(self,):
        ...
        self.n_vals_intra = self.gpu_n_vals_intra.get()[0]
        t1 = time.time()
        self.val_on_zero_inter_nuis = self.log_e * (self.n_pixl_sub_mat - self.n_vals_intra) *\
                                         -1.0 * self.param_simu_test['v_inter'][0]
    ...
    def approx_single_likelihood_on_zeros_mut(self,):
        ...
        t0 = time.time()
        start.record()
        self.kern_eval_likelihood_zeros(self.gpu_vect_id_c_mut,
        ...
        self.n_vals_intra = self.gpu_n_vals_intra.get()[0]
        t1 = time.time()
        self.val_on_zero_inter_mut = self.log_e * (self.n_pixl_sub_mat - self.n_vals_intra) *\
                                         -1.0 * self.param_simu['v_inter'][0]
    ...
    def approx_all_likelihood_on_zeros(self,):
        ...
        t0 = time.time()
        start.record()
        self.kern_eval_all_likelihood_zeros_1st(self.collect_gpu_vect_id_c,
        ...
        start.record()
        self.kern_eval_all_likelihood_zeros_2nd(self.gpu_list_uniq_mutations,
                                                self.gpu_n_uniq,
                                                self.gpu_param_simu,
                                                self.gpu_vect_likelihood_z,
                                                self.gpu_all_n_vals_intra,
                                                self.gpu_n_pixl_sub_mat,
                                                block=(self.n_threads_mutations, 1, 1), grid=(1, 1))
        end.record()
        end.synchronize()
        secs = start.time_till(end) * 1e-3
    ...
    def fill_dist_all_mut(self,):
        ...
        for id_mut in range(0, self.n_tmp_struct):
            start.record()
            self.kern_fill_vect_dist_all(self.gpu_sub_frags_2_frags,
            ...
            secs = start.time_till(end) * 1e-3
            # print "CUDA clock execution timing (compute sub vect distances): ", secs
    ...
    def fill_dist_single(self,):
        ...
        #####################################################################################
        start.record()
        self.kern_fill_vect_dist_single(self.gpu_sub_frags_2_frags,
    ...
    def slice_sparse_mat(self, id_ctg1, id_ctg2, id_fragA, id_fragB):
        ...
        self.ctx.synchronize()
        start.record()
        self.kern_slice_sp_mat(self.gpu_sp_no_rep_data,
        ...
        elapsed_seconds_splice = end.time_since(start) * 1e-3
        ################################################################################################################
        t_thrust_0 = time.time()
        self.thrust_module.sort_by_keys_zip(self.gpu_sub_sp_no_rep_rows, int(self.n_sub_vals),
                                            self.gpu_sub_sp_no_rep_cols, self.gpu_sub_sp_no_rep_data)
        t_thrust_1 = time.time()
        ################################################################################################################
        ...
        start.record()
        self.kern_prepare_sparse_call(self.gpu_sub_sp_no_rep_rows,
        ...
        elapsed_seconds_prepare = end.time_since(start) * 1e-3
        ################################################################################################################
        t_end = time.time()
        # print "GPU all splice: elapsed time  = ", t_end - t_start
        # print "GPU Thrust sort: elapsed time  = ", t_thrust_1 - t_thrust_0
        # print "GPU splice init : elapsed time = ", elapsed_seconds_splice
        # print "GPU splice prepare call : elapsed time = ", elapsed_seconds_prepare
    ...
    def show_sub_slice(self, id_ctg1, id_ctg2, id_frag_a, id_frag_b):
        ...
        self.np_sub_mat = scp.sparse.coo_matrix((dat, (rows, cols)), shape=(self.init_n_sub_frags, self.init_n_sub_frags))
    ...
    def eval_all_sub_likelihood(self,):
        ...
        #####################################################################################
        start.record()
        self.kern_sub_likelihood(self.gpu_sub_sp_no_rep_data,
                                 self.gpu_info_blocks,
                                 self.gpu_sub_sp_block_indptr,
                                 self.gpu_sub_sp_no_rep_rows,
                                 self.gpu_sub_sp_no_rep_cols,
        ...
        start.record()
        self.kern_eval_all_scores(self.gpu_list_uniq_mutations,
                                  self.gpu_n_uniq,
                                  self.gpu_vect_likelihood_z,
                                  self.gpu_sub_vect_likelihood_nz,
    ...
    def extract_current_sub_likelihood(self):
        ...
        self.ctx.synchronize()
        start.record()
        self.kern_extract_sub_likelihood(self.gpu_sub_sp_no_rep_data,
                                         self.gpu_info_blocks,
                                         self.gpu_sub_sp_block_indptr,
                                         self.gpu_sub_sp_no_rep_rows,
                                         self.gpu_sub_sp_no_rep_cols,
        ...
        secs = start.time_till(end) * 1e-3
        # print "CUDA clock execution timing(extract sub likelihood computing): ", secs
    ...
    def eval_likelihood_init(self,):
        ...
        start.record()
        self.kern_evaluate_likelihood_single(self.gpu_sp_no_rep_data,
                                             self.gpu_sp_no_rep_rows,
                                             self.gpu_sp_no_rep_cols,
                                             self.gpu_id_single,
                                             self.gpu_param_simu,
    ...
    def eval_likelihood_on_mut(self, id_mut):
        ...
        #####################################################################################
        # likelihood_on_nz = ga.sum(self.gpu_val_likelihood_4_mut, dtype=np.float64).get()
    ...
    def step_sampler(self, id_frag, n_neighbours, dt):
        ...
        flip_eject = 1
        for (id_cand, i) in zip(self.candidates, range(0, n)):
        ...
        id_f_sampled = self.candidates[global_id / self.n_tmp_struct]
        ...
        self.likelihood_t = o
        return o, dist, op_sampled, id_f_sampled, self.mean_length_contigs, self.n_contigs
    ...
    def step_sampler_debug(self, id_frag, n_neighbours):
        ...
        self.candidates = range(0, n_neighbours + 1)
    ...
    def extract_uniq_mutations(self, id_fi, id_fj, flip_eject):
        ...
        self.kern_uniq_mutations(self.gpu_vect_frags.get_ptr(),
                                 np.int32(id_fi),
                                 np.int32(id_fj),
                                 self.gpu_list_uniq_mutations,
    ...
    def loadProgram(self, filename):
        ...
        f.close()
        if self.active_insert_blocks:
            print "size array in shared memory = ", str(self.n_tmp_struct * self.size_block_4_sub)
            fstr = raw_fstr.replace("N_STRUCT_BY_BLOCK_SIZE", str(self.n_tmp_struct * self.size_block_4_sub)).\
                replace('SIZE_BLOCK_4_SUB_3', str(self.size_block_4_sub * 3)).\
                replace("N_TMP_STRUCT", str(self.n_tmp_struct)).replace('SIZE_BLOCK_4_SUB', str(self.size_block_4_sub)).\
        ...
        #create the program
        self.module = pycuda.compiler.SourceModule(fstr, no_extern_c=True,
                                                   options=[
        ...
        # self.sub_evaluate_likelihood_sparse = self.module.get_function('sub_evaluate_likelihood_sparse')
    ...
    def pop_out_pop_in(self, id_f_pop, id_f_ins, mode, max_id):
        ...
        self.pop_out(self.pop_gpu_vect_frags.get_ptr(), self.gpu_vect_frags.get_ptr(), self.pop_gpu_id_contigs,
                     np.int32(id_f_pop), np.int32(max_id), self.n_new_frags,
                     block=block_, grid=grid_)
        end.record()
        end.synchronize()
        secs = start.time_till(end)*1e-3
    ...
    def transloc(self, id_fA, id_fB, max_id):
        ...
        for upstreamfA in range(0, 2):
            ...
            self.split(self.trans1_gpu_vect_frags.get_ptr(), self.gpu_vect_frags.get_ptr(), self.trans1_gpu_id_contigs,
                       np.int32(id_fA), np.int32(upstreamfA), np.int32(max_id), self.n_new_frags,
                       block=block_, grid=grid_)
            end.record()
            end.synchronize()
    ...
    def insert_blocks(self, id_fA, id_fB, max_id):
        ...
        self.ctx.synchronize()
        self.kern_get_bounds(self.gpu_vect_frags.get_ptr(),
                             np.int32(id_fA),
                             np.int32(id_fB),
                             self.gpu_list_valid_insert,
        ...
        for i in range(0, self.n_insert_blocks):
            for j in [1, 0]:
                ...
                ########################################################################################################
                start.record()
                self.kern_extract_block(self.trans1_gpu_vect_frags.get_ptr(),
                                        self.gpu_vect_frags.get_ptr(),
                ...
                ########################################################################################################
                start.record()
                self.kern_insert_block(self.collector_gpu_vect_frags[id_start_insert + id].get_ptr(),
    ...
    def bomb_the_genome(self,):
        ...
        start.record()
        self.kern_explode_genome(self.gpu_vect_frags.get_ptr(),
                                 gpu_shuffle_order,
                                 self.n_new_frags,
                                 block=block_, grid=grid_)
        end.record()
    ...
    def setup_model_parameters(self, param, d_max):
        ...
        fact = np.float32(fact)
        p = np.array([(d0, d_max, alpha_0, alpha_1, fact, self.mean_value_trans)], dtype=self.param_simu_exp)
    ...
    def estimate_parameters_rippe(self, max_dist_kb, size_bin_kb, display_graph):
        ...
        for k in self.bins:
            self.dict_collect[k] = []
        for i in xrange(0, self.n_frags / 10):
            # print "frag i = ", i
            ...
            # local_bins = np.arange(size_bin_kb, min(len_kb_c_i, max_dist_kb) + size_bin_kb, size_bin_kb)
        ...
        self.mean_value_trans = self.mean_value_trans / 1.0
        ##########################################
        ...
        if display_graph:
            ...
            plt.title(r'$\mathrm{Frequency\ of\ contact\ versus\ genomic\ distance\ (data\ tricho test):}\ slope=%.3f,\ max\ cis\ distance(kb)=%.3f\ d=%.3f\ scale\ factor=%.3f\ $' %(self.param_simu['slope'], estim_max_dist, self.param_simu['d'], self.param_simu['fact']))
            plt.legend(['obs', 'fit'])
            plt.show()
    ...
    def load_gl_cuda_vbo(self,):
         #CUDA Ressorces
        self.pos_vbo.bind()
        #replace this with buffers[0] if pyopengl_accelerate is disabled
        #try:
        ...
        #    self.gpu_pos = cudagl.RegisteredBuffer(long(self.pos_vbo.buffer), cudagl.graphics_map_flags.NONE)
        #    self.gpu_col = cudagl.RegisteredBuffer(long(self.col_vbo.buffer), cudagl.graphics_map_flags.NONE)
        self.col_vbo.bind()
    ...
    def modify_gl_cuda_buffer(self, id_fi, dt):
        ...
        self.ctx.synchronize()
        self.kern_select_uniq_id_c(self.gpu_vect_frags.get_ptr(),
                                   self.gpu_uniq_id_c,
                                   self.gpu_uniq_len,
                                   self.gpu_counter_select_gl, # retrieve how many contigs are in the genome
                                   self.n_new_frags,
                                   block=block_, grid=grid_)
        end.record()
        end.synchronize()
        ...
        self.thrust_module.sort_by_keys_simple(self.gpu_uniq_len, int(self.n_contigs), self.gpu_uniq_id_c) # sort the contigs by length
        ...
        start.record()
        self.kern_make_old_2_new_id_c(self.gpu_uniq_id_c,
                                      self.gpu_old_2_new_id_c, # structure to update the id of the contigs
        ...
        self.ctx.synchronize()
        start.record()
        self.kern_count_vals(self.gpu_uniq_len,
                             np.int32(1),
                             self.gpu_counter_select_gl, # collect how many contigs have length 1
                             np.int32(self.n_contigs),
                             block=block_, grid=grid_)
        end.record()
        end.synchronize()
        ...
        #self.gl_update_pos(self.gpu_uniq_len,
        #                   np.intp(pos_ptr),
        #                   np.intp(col_ptr),
        #                   self.gpu_vel,
        #                   self.pos_gen_cuda,
        ...
        #self.kern_frags_2_gl_pxl(self.gpu_vect_frags.get_ptr(),
        ...
        # block_ = (size_block_4_gl, 1, 1)
        # grid_ = (self.n_data_4_gl / size_block_4_gl + 1, 1)
        # print "grid = ", grid_
        #t0 = time.time()
        #self.gpu_counter_select_4_gl.fill(0)
        #self.ctx.synchronize()
        #self.kern_prepare_sparse_call_4_gl(self.gpu_rows_4_gl,
        #                                   self.gpu_cols_4_gl,
        #                                   self.gpu_ptr_4_gl,
        #                                   self.gpu_vect_gl_pxl_frag,
        #                                   self.gpu_info_blocks,
        ...
        #self.ctx.synchronize()
        #self.kern_update_matrix(self.gpu_rows_4_gl,
        #                        self.gpu_cols_4_gl,
        #                        self.gpu_data_4_gl,
        #                        self.gpu_ptr_4_gl,
        #                        self.gpu_info_blocks,
        ...
        #grid_all = (all_pix_gl / 1024 + 1, 1)
        #self.kern_update_gl_buffer(np.intp(im_2_update),
        #                           self.gpu_im_gl,
        #                           np.int32(self.im_thresh),
        #                           np.int32(all_pix_gl),
        #                           block=(1024, 1, 1), grid=grid_all)
        #self.ctx.synchronize()
        #mapping_obj.unmap() # Unmap the GlBuffer
        #t1 = time.time()
    ...
    def test_thrust(self):
        t_start = time.time()
        self.thrust_module.sort_by_keys_zip(self.gpu_sub_sp_no_rep_rows, int(self.n_sub_vals), self.gpu_sub_sp_no_rep_cols)
        t_end = time.time()
        print "GPU thrust sort 1: elapsed time  = ", t_end - t_start
        self.thrust_module.sort_by_keys_zip(self.gpu_sub_sp_no_rep_rows, int(self.n_sub_vals), self.gpu_sub_sp_no_rep_data)
        t_end_2 = time.time()
    ...
    def f_rippe(self, x, param):
        ...
        if x < d_max:
            rippe = fact * (0.53 * (kuhn ** -3.) * np.power((lm * x/kuhn), slope) *
    ...
    def setup_distri_frags(self,):
        ...
        # print "shape indices = ", self.sub_sampled_sparse_matrix.indices.shape
    ...
    def return_neighbours(self, id_fA, delta0):
        ...
        if self.distri_frags[ori_id]['distri'] != None:
