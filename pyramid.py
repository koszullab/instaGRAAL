...
def remove_problematic_fragments(
    ...
    p = ProgressBar("blue", width=20, block="▣", empty="□")
    step = 0
    for id_line_contacts in range(1, len(all_lines_contact)):
...
class pyramid:
    def __init__(self, pyramid_folder, n_levels):
        ...
        for i in range(0, n_levels):
            ...
            frag_dictionary, contig_dictionary, list_contigs, list_contigs_id = self.build_frag_dictionnary(
...
class level:
    ...
    def load_data(self, pyramid):
        ...
        self.pos_vect_frags_4_GL = np.ndarray(
            (self.n_frags, 4), dtype=np.float32
        )
        ...
        for id_cont in list_contigs_id:
            ...
            for id_f in range(0, lim_curr_cont):
                ...
                self.S_o_A_frags["sub_len"].append(n_accu_frags)
        ...
        for id_cont in range(1, len(self.dict_contigs) + 1):
            ...
            total_trans -= n_intra
            n_tot += (
                sub_sp_mat_tmp.shape[0] * sub_sp_mat_tmp.shape[1]
