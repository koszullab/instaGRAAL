...
def subsample_data_set(contig_info, fragments_list, fact_sub_sample, abs_fragments_contacts, new_abs_fragments_contacts_file,
    ...
    if fact_sub_sample <= 1:
        ...
        if not(abs_fragments_contacts == 'SIMU'):
            ...
            sparse_dict = dict()
            for line in handle_abs_fragments_contacts:
                dat = line.split('\t')
                fa = int(dat[0])
                fb = int(dat[1])
                nc = int(dat[2])
...
def remove_problematic_fragments(contig_info, fragments_list, abs_fragments_contacts,new_contig_list_file,
    ...
    plt.figure()
    plt.scatter(range(len(collect_sparsity)), collect_sparsity)
    plt.savefig("sparsity_plot.pdf")
    plt.close()
    plt.figure()
    dist_sparsity = collections.Counter(collect_sparsity).most_common()
    plt.scatter(dist_sparsity.keys(), dist_sparsity.values())
    plt.savefig("sparsity_dist.pdf")
    ...
    thresh = mean_spars - 2.01 * std_spars
    ...
    print("I have to remove "+str(len(list_fragments_problem))+" fragments")
...
class level():
    ...
    def load_data(self, pyramid):
        ...
        self.pos_vect_frags_4_GL = np.ndarray((int(self.n_frags), 4), dtype=np.int32)
        self.col_vect_frags_4_GL = np.ndarray((int(self.n_frags), 4), dtype=np.int32)
