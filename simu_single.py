...
class simulation():
    def __init__(self, name, level, n_iterations, is_simu, gl_window, use_rippe, gl_size_im, size_pyramid=7):
        self.name = name
        self.use_rippe = use_rippe
        self.str_sub_level = str(level - 1)
        self.str_level = str(level)
        self.select_data_set(name, size_pyramid=size_pyramid)
        ...
        ### DEFINE REPEATED SEQ ####
        self.candidate_dup , self.data_candidate_dup, self.sub_candidates_dup, self.sub_candidates_output_data = \
        ...
        self.sampler = sampler_lib(use_rippe, self.new_S_o_A_frags, self.collector_id_repeats, self.frag_dispatcher,
                                   ...
                                   self.norm_vect,
                                   self.sub_candidates_dup, self.sub_candidates_output_data,
                                   self.new_sub_S_o_A_frags, self.sub_collector_id_repeats, self.sub_frag_dispatcher,
                                   ...
                                   self.raw_im_init, self.pbo_im_buffer, self.gl_size_im)
        ...
        print "min fragment length = ", self.new_sub_S_o_A_frags['len_bp'].min() / 1000.0
        if is_simu:
            self.sampler.simulate_rippe_contacts(100, 9.6, -1.5, 0.5, 1, 800, 200)
        else:
            if self.use_rippe:
                self.sampler.estimate_parameters_rippe(max_dist_kb, mean_size_bin_kb / 2., display_graph)
            else:
                self.sampler.estimate_parameters(max_dist_kb, mean_size_bin_kb / 2, display_graph)
    ...
    def modify_vect_frags(self):
        ...
        for data_dup in self.data_candidate_dup:
            ...
            for k in range(0, n_dup):
                ...
                modified_vect_frags['sub_len'].append(init_vect_frags['sub_len'][id_f])
                ...
                modified_vect_frags['sub_l_cont'].append(init_vect_frags['sub_len'][id_f])
        ...
        modified_vect_frags['sub_pos'] = np.array(modified_vect_frags['sub_pos'], dtype=np.int32)
        ...
        modified_vect_frags['sub_len'] = np.array(modified_vect_frags['sub_len'], dtype=np.int32)
        ...
        modified_vect_frags['sub_l_cont'] = np.array(modified_vect_frags['sub_l_cont'], dtype=np.int32)
    ...
    def modify_sub_vect_frags(self):
        ...
        collector_id_repeats = []
        frag_dispatcher = []
        for id_f in xrange(0, self.init_n_sub_frags):
        ...
        self.sub_collector_id_repeats = np.array(collector_id_repeats, dtype=np.int32)
    ...
    def select_repeated_frags(self): #
        ...
        #         p.render(pt, 'step %s\nProcessing...\nDescription: computing coverage per frag.' % step)
        ...
        # # plt.show()
        # plt.figure()
        # n, bins, patches = plt.hist(coverage, 10000, normed=1, facecolor='blue', alpha=0.75)
        # # add a 'best fit' line
        ...
        # print "duplicated data = ", output_data
            ...
            for i in range(0, n_subs):
                ...
                sub_candidates_output_data.append((tmp_sub_ids[i], estim_n_dup))
        print "N frag duplicated = ", len(candidates_dup)
        return candidates_dup, output_data, sub_candidates_dup, sub_candidates_output_data
    ...
    def select_data_set(self, name, size_pyramid=7):
        ...
        if hostname == 'duvel':
            ...
            if selected == 'tricho_qm6a_sparse':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/trichoderma/trichoderma_new.fa')
            elif selected == 'S1':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/cerevisiae_classic/new_ref_genome.fsa')
            elif selected == 'amoeba':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/amoeba/EHI_v13.fa')
            elif selected == 'mosquito':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/aedes_aegypti/Aedes_aegypti.fa.parsed')
            elif selected == 'de_novo_chr14' or selected == 'de_novo_chr14_jff':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/de_novo_chr14/genome.ctg.fasta')
            elif selected == 'human_chr4_chr14':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/human_chr4_chr14/human_4_14.fa')
            elif selected == 'human_chr19_to_22':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/human_chr19_to_22/chr19_chr20_chr21_chr22.fa')
            elif selected == 'human_chr7_chr22':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/human_chr7_chr22/chr7_chr17_chr19_chr22.fa')

            elif selected == 's1_de_novo' or selected == 's1_de_novo_0_1':
                self.fasta = os.path.join(self.dir_home, 'fasta_genomes/cerevisiae_de_novo/cerevisiae_de_novo.fa')
            ...
            if selected == 'mosquito':
                ...
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/trichoderma/trichoderma_new.fa')
            elif selected == 'S1':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/cerevisiae_classic/new_ref_genome.fsa')
            elif selected == 'amoeba':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/amoeba/EHI_v13.fa')
            elif selected == 'mosquito':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/aedes_aegypti/Aedes_aegypti.fa.parsed')
            elif selected == 'de_novo_chr14' or selected == 'de_novo_chr14_fjff':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/de_novo_chr14/genome.ctg.fasta')
            elif selected == 'human_chr4_chr14':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/human_chr4_chr14/human_4_14.fa')
            elif selected == 'human_chr19_to_22':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/human_chr19_to_22/chr19_chr20_chr21_chr22.fa')
            elif selected == 'human_chr7_chr22':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/human_chr7_chr22/chr7_chr17_chr19_chr22.fa')


            elif selected == 's1_de_novo' or selected == 's1_de_novo_0_1':
                self.fasta = os.path.join(self.dir_home, 'alignment_toolbox', 'fasta_genomes/cerevisiae_de_novo/cerevisiae_de_novo.fa')
            else:
                self.fasta = os.path.join(toolbox_directory, 'fasta', selected+'.fa')
    ...
    def init_gl_image(self,):
        ...
        # self.raw_im_init = np.uint8(np.random.rand(self.gl_size_im, self.gl_size_im))
        self.raw_im_init = np.zeros((self.gl_size_im,self.gl_size_im), dtype=np.uint8)

        glBufferData(GL_PIXEL_UNPACK_BUFFER, self.gl_size_im * self.gl_size_im, self.raw_im_init, GL_STREAM_DRAW) # Allocate the buffer
...
#            print str(e)
        ...
        # glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, self.gl_size_im, self.gl_size_im,  0, GL_LUMINANCE, GL_FLOAT, self.raw_im_init) # Allocate the texture
        glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, self.gl_size_im, self.gl_size_im,  0, GL_LUMINANCE, GL_UNSIGNED_BYTE, None) # Allocate the texture
    ...
    def create_sub_frags(self):
        ...
        for i in xrange(0, self.n_frags):
            ...
            for j in xrange(0, n_sub):
                v_len[j] = np.float32(self.sub_level.vect_frag_np[tmp[0] + j]['len_bp']) / unkb
                v_id[j] = np.int32(tmp[0] + j)
                v_accu[j] = np.int32(self.sub_level.vect_frag_np[tmp[0] + j]['n_accu'])
            ...
            for j in xrange(0, n_sub):
                ...
                c_d = (np.sum(tmp_len[range(n_sub - 1, j, -1)]) + tmp_len[j]/2.) # crick distance
                # dat = (i, w_d, c_d)
                dat = (i, w_d, c_d, j)
        ...
        # self.np_sub_frags_2_frags = np.array(self.sub_frags_2_frags, dtype=self.float3)
        self.np_sub_frags_2_frags = np.array(self.sub_frags_2_frags, dtype=self.float4)
