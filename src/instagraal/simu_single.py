...
class simulation:
    def __init__(
        ...
        logger.info("mean frag area = {}".format(self.mean_squared_frags_per_bin))
        ...
        max_dist_kb = self.sampler.gpu_vect_frags.l_cont_bp[id_start].max() / 1000.0
        ...
        logger.info("min fragment length =  {}".format(self.new_sub_S_o_A_frags["len_bp"].min() / 1000.0))
        if is_simu:
            self.sampler.simulate_rippe_contacts(100, 9.6, -1.5, 0.5, 1, 800, 200)
        else:
            if self.use_rippe:
                self.sampler.estimate_parameters_rippe(max_dist_kb, mean_size_bin_kb / 2.0, display_graph)
            else:
    ...
    def discard_low_coverage_frags(self):
        ...
        logger.info("n discarded frag of low coverage = {}".format(candidates_low.shape[0]))
    ...
    def modify_vect_frags(self):
        ...
        for id_f_curr in range(0, self.n_frags):
            ...
            pos_vect_frags_4_GL[id_f_curr, 2] = 0.0
    ...
    def select_data_set(self, name):
        ...
        if not (os.path.exists(self.output_folder)):
            os.mkdir(self.output_folder)
        self.output_folder = os.path.join(self.output_folder, "test_mcmc_" + self.str_level)
        if not (os.path.exists(self.output_folder)):
            os.mkdir(self.output_folder)
    ...
    def load_gl_buffers(self):
        ...
        self.vel[:, 2] = self.pos[:, 2] * 2.0
        self.vel[:, 1] = self.pos[:, 1] * 2.0
        self.vel[:, 0] = 3.0
    ...
    def init_gl_image(
        ...
        self.texid = 0
        self.pbo_im_buffer = OpenGL.GL.glGenBuffers(1)  # generate 1 buffer reference

        OpenGL.GL.glBindBuffer(OpenGL.GL.GL_PIXEL_UNPACK_BUFFER, self.pbo_im_buffer)  # binding to this buffer
        ...
        OpenGL.GL.glBindTexture(OpenGL.GL.GL_TEXTURE_2D, self.texid)  # binding to this texture
    ...
    def create_sub_frags(self):
        ...
        for i in range(0, self.n_frags):
            tmp = [
                self.hic_pyr.spec_level[self.str_level]["fragments_dict"][i + 1]["sub_low_index"] - 1,
                self.hic_pyr.spec_level[self.str_level]["fragments_dict"][i + 1]["sub_high_index"] - 1,
            ]
            ...
            for j in range(0, n_sub):
                w_d = np.sum(tmp_len[0:j]) + tmp_len[j] / 2.0  # watson distance
                c_d = np.sum(tmp_len[list(range(n_sub - 1, j, -1))]) + tmp_len[j] / 2.0  # crick distance
                # dat = (i, w_d, c_d)
                dat = (i, w_d, c_d, j)
